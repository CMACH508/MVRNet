...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(cfg.SOLVER.START_EPOCHS, cfg.SOLVER.EPOCHS):
            # self.task.adjust_learning_rate(epoch)
            torch.cuda.empty_cache()
            # train cnfig
            # self.task.lr_scheduler.step()
            print('\n[lpy] Epoch [{}/{}] start train'.format(epoch, cfg.SOLVER.EPOCHS))
            ...
            if ((epoch + 1) < eval_start_epoch) or (epoch + 1) % eval_frequence != 0:
                continue
            print('[lpy] Epoch [{}/{}] start validate'.format(epoch, cfg.SOLVER.EPOCHS))
            vv = self.task.validate()
