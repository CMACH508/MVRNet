...
def progress_monitor(total, verbose=1):
    ...
    def _print(*args, ):
        ...
        finished_count += 1
        if verbose>0 and finished_count % verbose == 0:
            print("progress: %d/%d = %.2f; time: %.2f s" % (
                finished_count, total, finished_count / total, time.time() - time_start))
        if finished_count == total:
            print('finished: %d, total cost time: %.2f s' % (total, time.time() - time_start,))
...
def eval_segmentation(subject, seg_result, other_infos, mask_dir, blood_dir, use_blood, save_seg, save_dir):
    ...
    gt = nib.load(mask_path).get_data()  ## lpy add
    # print('gt.shape={} gt.sum={}'.format(gt.shape, gt.sum()))
    # print('seg.shape={} seg.sum={}'.format(seg.shape, seg.sum()))
...
def evaluation(eval_results, image_dir, mask_dir, blood_dir='', use_blood=False, save_seg=False, save_dir=None):
    ...
    monitor = progress_monitor(total=len(eval_results), verbose=len(eval_results)//3)
