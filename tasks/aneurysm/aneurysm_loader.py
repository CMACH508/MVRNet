...
def read_data(subject):
    ''' input: pid of data, output: aneurysm image & lesion mask and enhanced
    ...
    aneurysm_path = os.path.join(cfg.TRAIN.DATA.ANEURYSM_FOLDER, '%s.nii.gz' % subject)
    if not os.path.exists(aneurysm_path):
        aneurysm_path = aneurysm_path.replace('.nii.gz', '_mask.nii.gz')
        # print('mask not found ---->', cfg.TRAIN.DATA.ANEURYSM_FOLDER)
    ...
    if cfg.TRAIN.DATA.IS_ENHANCED:
        enahnced_path = os.path.join(cfg.TRAIN.DATA.ENHANCED_FOLDER, '%s.nii.gz' % subject)
        enhanced = nib.load(enahnced_path).get_data()
    else:
    ...
    return img, aneurysm, spacing_zxy, enhanced


def sampe_subjects(subjects, aneurysm_bbox, patch_num):
    img_lst, gt_lst, enhanced_lst, delayed_transform_add_coords = [], [], [], []
    for subject in subjects:
        image, aneurysm, spacing_zxy, enhanced = read_data(subject)  # origin data  z,x,y
        img, gt, enhanced, _delayed_transform_add_coords = AneurysmSampler(subject, aneurysm_bbox[subject], spacing_zxy, image, aneurysm, enhanced).sample(patch_num)
        img_lst.append(img)
        gt_lst.append(gt)
    ...
    return np.concatenate(img_lst), np.concatenate(gt_lst), np.concatenate(enhanced_lst), delayed_transform_add_coords
...
class AneurysmDataset(Thread):
    ...
    def asyn_sample(self, epoch=-1, subject_num=100, patch_num=100, max_workers=20):
        indices = list(range(len(self.subjects)))
        random.shuffle(indices)
        indices = indices[:subject_num]

        monitor = progress_monitor(total=len(indices), verbose=10)
        ...
        print('data processing in async for epoch {}...'.format(epoch))

    def get_data_loader(self):
        '''wait for async processing finished, then extract return aneurysm image and mask data'''
        self.ex.shutdown(wait=True)
        img_lst, gt_lst, enhanced_lst, label_lst = [], [], [], []
        delayed_transform_add_coords = []
        for obj in self.objs:
            # print('----------- get -------- get -----------', len(obj.result()))
            img, gt, enhanced, _delayed_transform_add_coords = obj.result()
            img_lst.append(img)
            gt_lst.append(gt)
            enhanced_lst.append(enhanced)
        ...
        # img: b,c,d,w,h
        # gt: b,d,w,h
        img, gt, enhanced, label = np.concatenate(img_lst), np.concatenate(gt_lst), np.concatenate(enhanced_lst), np.concatenate(label_lst)

        print('concate need %.4f' % (time.time() - t0))
        para_dict = {"image": img, "gt": gt, "enhanced": enhanced, 'label:': label, \
                     'delayed_transform_add_coords': delayed_transform_add_coords}
        return ANEURYSM_SEG(para_dict, "train")


class AneurysmSampler(object):
    __slots__ = 'subject','image','aneurysm','enhanced','img_size','patch_size','offset',\
                'bbox_list','spacing_zxy','is_neck','add_coords','slice_threshold','slice_zs','slice_ze'
    def __init__(self, subject, bbox_list, spacing_zxy, image, aneurysm, enhanced, is_neck=True, add_coords=False):
        ...
        self.enhanced = self.enhanced[np.newaxis, np.newaxis]
    ...
    def sample(self, k):
        ...
        for x0,y0,z0 in positive_coords_list:
            ...
            # print('samplimg pos img.shape={}'.format(img.shape))  # (1, 3, 80, 80, 80)

            vesl = self.enhanced[..., x0:x1, y0:y1, z0:z1]
            if cfg.MODEL.NAME == 'dlia_datacat4_add':
                vesl1 = set_window_wl_ww(vesl, ww=100, wl=50)  # 1,1,Z,XY
                vesl2 = set_window_wl_ww(vesl, ww=100, wl=150)  # 1,1,Z,XY
                vesl3 = set_window_wl_ww(vesl, ww=600, wl=500)  # 1,1,Z,XY
                vesl = np.concatenate([vesl1, vesl2, vesl3], axis=1)
            else:
                vesl = set_window_wl_ww(vesl, ww=ww, wl=wl)  # 1,1,Z,XY
            vesl = (vesl / 255.0) * 2.0 - 1.0
            if self.add_coords:
        ...
        for x0,y0,z0 in negative_coords_list:
            ...
            # img3 = set_window_wl_ww(img, ww=500, wl=600)  # 1,1,Z,XY
            # img = np.concatenate([img1, img2, img3], axis=1)
            # print('samplimg neg img.shape={}'.format(img.shape))  # (1, 3, 80, 80, 80)
        ...
        img_lst, gt_lst, enhanced_lst = np.concatenate(img_lst), np.concatenate(gt_lst), np.concatenate(enhanced_lst)

        return img_lst, gt_lst, enhanced_lst, delayed_transform_add_coords