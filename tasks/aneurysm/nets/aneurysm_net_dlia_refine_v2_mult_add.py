...
class DAResUNet(nn.Module):
    ...
    def __init__(self, segClasses=2, k=16, input_channel=1, psp=False):
        ...
        print('----------------- import refine_multi_view_add ---------------')
    ...
    def forward(self, x, out1_0, out2_0, out3_0, out4_0, draw):
        ...
        if draw:
            ...
            # intm_out = []
            # for ii, intm in enumerate(intm_list):
            #     # print("max: {}, min: {}".format(intm.max(), intm.min()))
            #     intm_tmp = intm[0].mean(0)
            #     # print(intm_tmp.shape)
            #     intm_tmp = intm_tmp[intm_tmp.shape[0] // 2]
            #     # start, end = start_end[ii]
            #     # if start == 0:
            #     #     intm_tmp = intm_tmp
            #     # else:
            #     #     intm_tmp = intm_tmp[start:end, start:end]
            #     # print(intm_tmp.shape)
            #     intm_out.append(intm_tmp)
            return {'y': output, 'intm_list': intm_list}

        return {'y': output}

if __name__ == '__main__':
    aa = torch.rand(size=(1, 2, 80, 80, 80))
    model = DAResUNet(k=16, input_channel=2)
    out = model(aa, None, None, None, None)['y']
