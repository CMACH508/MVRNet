...
class Task(object):
    ...
    def __init__(self):
        ...
        print('lr mode=', cfg.SOLVER.LR_MODE)
        print('lr=', self.lr_scheduler.get_lr()[0])
    ...
    def model_pretrain(self, pretrain):
        print('-------------- enter model_pretrain ------------')
        if os.path.isfile(pretrain):
            self.logger.info("=> loading pretrained checkpoint '{}'".format(pretrain))
    ...
    def model_resume(self, save_dir):
        ...
        for m in self.metrics:
            ...
            if os.path.exists(ct_best_pth):
                ct_best = torch.load(ct_best_pth, map_location='cpu') 
